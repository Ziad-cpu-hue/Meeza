...
class TripCreateSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        extra_kwargs = {
            # لازم إحداثيات دقيقة لنقطتي الانطلاق والوصول — مينفعش نعتمد على اسم المنطقة بس
            "pickup_lat": {"required": True, "allow_null": False},
            "pickup_lng": {"required": True, "allow_null": False},
            "dropoff_lat": {"required": True, "allow_null": False},
            "dropoff_lng": {"required": True, "allow_null": False},
        }
...
class AdjustPriceSerializer(serializers.Serializer):
    """تعديل حر للسعر (زيادة أو نقصان) بخطوة جنيه واحد على الأقل — يستخدمه زرا +/- الفوريين."""
    delta_amount = serializers.DecimalField(max_digits=8, decimal_places=2)

    def validate_delta_amount(self, value):
        if value == 0:
            raise serializers.ValidationError("قيمة التعديل لازم تكون مختلفة عن صفر")
        return value


class AvailableCaptainSerializer(serializers.Serializer):
    """كابتن متاح/متصل حالياً ومؤهل لاستلام هذا الطلب — يُعرض للعميل أثناء انتظار القبول."""
