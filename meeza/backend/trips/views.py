...
from .serializers import (
    TripCreateSerializer, TripSerializer, IncreasePriceSerializer, AdjustPriceSerializer,
    AvailableCaptainSerializer, DriverLocationSerializer, PaymentProofSerializer,
)
...
class AvailableCaptainsView(APIView):
    """
    قائمة الكباتن المتصلين والمؤهلين لاستلام رحلة معينة (بانتظار القبول) — تُعرض للعميل
    أثناء الانتظار عشان يشوف مين شايف طلبه ومتاح يقبله دلوقتي.
    """
    permission_classes = [IsCustomer]
    ...
    def get(self, request, pk):
        ...
        vehicle_type = SERVICE_TO_VEHICLE.get(trip.service_type)
        captains = DriverApplication.objects.filter(
            status=DriverApplication.Status.APPROVED,
            is_online=True,
            vehicle_type=vehicle_type,
        ).select_related("user")
        captains = [c for c in captains if not c.is_blocked_for_debt]
        return Response(AvailableCaptainSerializer(captains, many=True).data)


class AdjustTripPriceView(APIView):
    """
    تعديل فوري وحر لسعر الرحلة أثناء الانتظار — زيادة أو نقصان بخطوة جنيه واحد
    (أو أي قيمة) بدون أي تأخير، يستخدمه زرا +/- في واجهة العميل.
    """
    permission_classes = [IsCustomer]
    ...
    def post(self, request, pk):
        ...
        serializer.is_valid(raise_exception=True)
        delta = serializer.validated_data["delta_amount"]

        # منع تنزيل البونص لأقل من صفر (السعر الأساسي المحسوب من المسافة ثابت ومينفعش ينزل تحته)
        new_bonus = trip.bonus_amount + delta
        if new_bonus < 0:
            return Response({"detail": "مينفعش تنزل السعر تحت السعر الأساسي للرحلة"}, status=400)

        trip.bonus_amount = new_bonus
        trip.total_price += delta
        trip.driver_profit += delta
        if delta > 0:
            trip.price_increase_count += 1
        trip.save()
        return Response(TripSerializer(trip).data)
...
class StartTripView(APIView):
    """
    الكابتن يضغط 'استلمت العميل' لما يوصله فعلياً — بيحوّل الرحلة من accepted لـ ongoing،
    وده اللي بيخلي الخريطة تبدأ ترسم المسار من الكابتن للوجهة النهائية بدل موقع العميل.
    """
    permission_classes = [IsDriver]
    ...
    def post(self, request, pk):
        try:
            ...
            return Response({"detail": "الرحلة غير موجودة أو تم استلام العميل بالفعل"}, status=400)
    ...
    def post(self, request, pk):
        try:
            trip = Trip.objects.get(
                ...
                status__in=[Trip.Status.ACCEPTED, Trip.Status.ONGOING],
            )
        except Trip.DoesNotExist:
...
class CompleteTripView(APIView):
    ...
    def post(self, request, pk):
        try:
            trip = Trip.objects.get(pk=pk, driver=request.user, status=Trip.Status.ONGOING)
        except Trip.DoesNotExist:
            return Response({"detail": "لازم تضغط 'استلمت العميل' الأول قبل إنهاء الرحلة"}, status=400)
