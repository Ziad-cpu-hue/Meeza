...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("savePricingModeBtn").addEventListener("click", savePricingMode);
  document.getElementById("savePricePerKmBtn").addEventListener("click", savePricePerKm);
...
async function loadPricing() {
  try {
    ...
    // وضع التسعير الحالي
    const modeInput = document.querySelector(`input[name="pricingMode"][value="${cfg.pricing_mode || "fuel_based"}"]`);
    if (modeInput) modeInput.checked = true;

    // سعر الكيلومتر الواحد لكل نوع مركبة
    const kmWrap = document.getElementById("pricePerKmFields");
    kmWrap.innerHTML = Object.entries(cfg.price_per_km || {}).map(([key, val]) => `
      <div class="field">
        <label>${vehicleTypeLabelsAdmin[key] || key}</label>
        <input type="number" step="0.1" min="0" class="km-price-input" data-key="${key}" value="${val}">
      </div>
    `).join("");
  } catch (err) {
    showToast(err.message, "err");
...
async function savePricingMode() {
  const mode = document.querySelector('input[name="pricingMode"]:checked')?.value;
  if (!mode) return showToast("اختار وضع تسعير الأول", "err");
  try {
    await apiRequest("/api/adminpanel/pricing-config/", {
      method: "PATCH",
    ...
    showToast(
      mode === "per_km"
        ? "تم التفعيل: هيتم حساب السعر الآن كـ (المسافة × سعر الكيلومتر) في كل الرحلات الجديدة"
        : "تم الرجوع لوضع حساب الوقود (الافتراضي) في كل الرحلات الجديدة",
      "ok"
    );
  } catch (err) {
    showToast(err.message, "err");
...
async function savePricePerKm() {
  const pricePerKm = {};
  document.querySelectorAll(".km-price-input").forEach((input) => {
    pricePerKm[input.dataset.key] = parseFloat(input.value);
  });
  try {
    await apiRequest("/api/adminpanel/pricing-config/", {
      method: "PATCH",
      body: { price_per_km: pricePerKm },
      admin: true,
    });
    showToast("تم حفظ أسعار الكيلومتر — هتتفعّل فوراً لو وضع التسعير مفعّل على 'سعر ثابت لكل كيلومتر'", "ok");
  } catch (err) {
    showToast(err.message, "err");
  }
}
